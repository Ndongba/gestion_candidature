    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-5 px-4 sm:px-8 mt-0">
        <div class="flex items-center py-4 bg-white border rounded-2xl shadow overflow-hidden">
            <div class="px-4 text-gray-700">
                ...
                <p class="text-3xl bold py-2">{{ $formation->formations->where('etat', 'accepter')->count() }}</p>
            </div>
        </div>
        <div class="flex items-center py-4 bg-white border rounded-2xl shadow overflow-hidden">
            <div class="px-4 text-gray-700">
                <h3 class="text-sm tracking-wider">Ca   ndidats en attente</h3>
                <p class="text-3xl bold py-2">{{ $formation->formations->where('etat', 'en_attente')->count() }}</p>
            </div>
        </div>
        <div class="flex items-center py-4 bg-white border rounded-2xl shadow overflow-hidden">
            <div class="px-4 text-gray-700">
                <h3 class="text-sm tracking-wider">Candidats non évalués</h3>
                <p class="text-3xl bold py-2">{{ $formation->formations->where('etat', 'en_evaluation')->count() }}
                </p>
            </div>
        </div>
        <div class="flex items-center py-4 bg-white border rounded-2xl shadow overflow-hidden">
            <div class="px-4 text-gray-700">
                ...
                <p class="text-3xl bold py-2">{{ $formation->formations->where('etat', 'refuser')->count() }}</p>
            </div>
        </div>
        <div class="flex items-center py-4 bg-white border rounded-2xl shadow overflow-hidden">
            <div class="px-4 text-gray-700">
                <h3 class="text-sm tracking-wider">Candidats postulés</h3>
                <p class="text-3xl bold py-2">{{ $formation->formations->count() }}</p>
            </div>
        </div>
                ...
                <tbody>
                    @foreach ($formations as $formation)
                        <tr class="hover:bg-gray-100 p transition-colors duration-200">
                            ...
                            <td class="border-t px-4 py-5">{{ $formation->biographie }}</td>
